...
export const useHandlePinnedProjectList = () => {
  ...
  const getPinnedProjects = useCallback(async () => {
    try {
      const projects = (await fetchPinnedProjects(userId, authContext?.user?.access_token)) as ProjectInfo[];
      if (projects?.length) {
        dispatch?.({
  ...
  const handlePinProject = useCallback(async (projectId: string) => {
    ...
    try {
      const newProject = await pinProject(projectId, authContext?.user?.access_token);
      if (newProject) {
        dispatch?.({
  ...
  const handleUnpinProject = useCallback(async (projectId: string) => {
    ...
    const toastId = uuidv4();
    const userId = 'me00247';
    const currentPinnedProjects = await fetchPinnedProjects(userId, authContext?.user?.access_token);
    ...
    apiCallTimeout = setTimeout(() => {
      unpinProject(userId, projectId, authContext?.user?.access_token).catch((error) => {
        dispatch?.({
          type: PROJECT_ACTION.INIT_PINNED_PROJECT_LIST,
          payload: currentPinnedProjects,
        } as ProjectActionType);
