...
describe('StdInputText', () => {
  ...
  const ControlledInputText = ({ onChange }: { onChange: StdChangeHandler<string> }) => {
  ...
  const UncontrolledInputText = ({ onChange }: { onChange: StdChangeHandler<string> }) => (
    <StdInputText label={TEST_LABEL} defaultValue={TEST_DEFAULT} onChange={(e) => onChange(e)} />
  );

  test.each([
    ['controlled', ControlledInputText],
    ['uncontrolled', UncontrolledInputText],
  ])('in %s mode, clear button should clear input, emit onChange and focus', async (_mode, Component) => {
    const user = userEvent.setup();
    const onChange = vitest.fn();
