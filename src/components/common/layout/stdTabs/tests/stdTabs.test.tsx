  ...
  it('renders StdTabs component with primary and secondary tablist with inactive classes', () => {
    ...
    render(
      <StdTabs
        items={items}
        id={TEST_ID}
        renderPrimary={(item) => <StdTabItem key={item.name} {...item} onClick={noop} id={TAB_ID} />}
        renderSecondary={(item) => <StdTabItem key={item.name} {...item} onClick={noop} id={TAB_ID_SECONDARY} />}
      />,
    );
    const tabItems = screen.getAllByRole('tab');
