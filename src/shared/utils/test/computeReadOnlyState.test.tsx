...
describe('computeReadOnlyState', () => {
  ...
  it('should disable Links if area status is ERROR', () => {
    const readOnly = computeReadOnlyState(dummyTrajectory, TRAJECTORY_SELECTION_STATUS.ERROR, StudyStatus.IN_PROGRESS, true);
    expect(readOnly).toEqual({ '0': false, '1': true });
  });

  it('should disable Links if no link trajectory and study is GENERATED', () => {
    const readOnly = computeReadOnlyState(dummyTrajectory, TRAJECTORY_SELECTION_STATUS.OK, StudyStatus.GENERATED, false);
    expect(readOnly).toEqual({ '0': false, '1': true });
  });

  it('should enable Links if area trajectory exists, no error, study is not GENERATED, and has link trajectory', () => {
    const readOnly = computeReadOnlyState(dummyTrajectory, TRAJECTORY_SELECTION_STATUS.OK, StudyStatus.IN_PROGRESS, true);
